...
NODE_USER="{{webapp_service_user}}"
NODE_EXECUTABLE="$NODE_BIN_DIR/node"
FOREVER_EXECUTABLE="$NODE_BIN_DIR/forever"
APPLICATION_PATH="{{webapp_directory}}/node_modules/.bin/nci"
LOGFILE="{{webapp_log_directory}}/nci.log"
MIN_UPTIME="5000"
SPIN_SLEEP_TIME="2000"
...
start() {
    ...
    COMMAND="$NODE_EXECUTABLE $FOREVER_EXECUTABLE \
    --uid \"{{webapp_service_user}}\" \
    ...
    su - "$NODE_USER" -c "$COMMAND" 2>&1 > /dev/null
...
stop() {
    ...
    COMMAND="$NODE_EXECUTABLE $FOREVER_EXECUTABLE stop $APPLICATION_PATH"
    su - "$NODE_USER" -c "$COMMAND" 2>&1 > /dev/null
    RETVAL=$?
...
status() {
    ...
    COMMAND="$NODE_EXECUTABLE $FOREVER_EXECUTABLE list"
    echo `su - "$NODE_USER" -c "$COMMAND"` | grep -q "$APPLICATION_PATH"
    if [ "$?" -eq "0" ]; then
